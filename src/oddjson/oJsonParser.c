...
{
   p->ownSca = make_scanner_c_( full.s, full.v );
   p->sca = &p->ownSca;
   p->err = cNoError_;
   skip_ws( p->sca );
   return true;
...
bool init_json_parser_shell_o( oJsonParser p[static 1],
                               cScanner sca[static 1] )
{
   p->ownSca = null_scanner_c_();
   p->sca = sca;
   p->err = cNoError_;
   skip_ws( p->sca );
   return true;
